...
> **YAML-First:** All feature status changes, spec creation, contract creation, demo recording, and backlog additions write to docs/yaml/*.yaml first. Markdown views are auto-generated by hooks.

Triggered by `/solo-dev:next-feature`. Runs phases 0-8 per feature with 3 user checkpoints.

```
/solo-dev:next-feature
  → Reads roadmap.md
  → Finds next QUEUED feature where all depends_on are COMPLETE
  → Checkpoint 1: Pre-Flight Briefing (confirm understanding, effort, assumptions)
  → Phases 0-1 (research + design — autonomous)
  → Checkpoint 2: Mid-Flight Review (confirm design spec before build)
  → Phases 2-7 (build + QA — autonomous, only critical blockers interrupt)
  → Checkpoint 3: Post-Flight Debrief (confirm ship, review deferred items, choose demo)
  → Phase 8 (demo + commit)
```

**3-Checkpoint Model:** S features skip Mid-Flight (2 checkpoints). M/L/XL use all 3. See orchestrator.md for full protocol.
...
- Add entry to docs/yaml/changelog.yaml with feature summary, changes list, breaking changes
- Regenerate CHANGELOG.md via yaml-to-markdown.sh

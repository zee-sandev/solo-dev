...
description: Every feature goes through up to 8 phases with 3 user checkpoints before shipping.
---
...
(confirm idea)                  (confirm design)                (confirm ship)
```
...
`backend-agent` writes API contracts first → other agents validate before building (see [Agent Feedback Protocol](/docs/architecture/agent-feedback-protocol) L1).

<Callout title="Foundation Projects">
...
Builds the project, starts the dev server, and tests critical endpoints to verify the implementation runs correctly at runtime.

**What it checks:**
1. Project builds without errors
2. Dev server starts within `smoke_test.timeout` seconds
3. Critical happy-path endpoints respond correctly
4. Error paths respond correctly (auth fail, invalid input, 404)

On FAIL → targeted feedback to specific implementation agents → fix → re-smoke.
...
Verifies that API contracts defined at the start of implementation have not changed during the implementation phase.
...
**Condition:** `design_profile` configured AND `visual_qa.enabled: true`

Captures screenshots of all new/changed pages and verifies visual quality against the user's design profile.
...
8. **Navigation** — matches `design_profile.navigation`

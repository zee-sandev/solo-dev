---
title: Commands
description: All 13 solo-dev commands for the full product development lifecycle.
---
...
**Path B (existing codebase):** Analyzes codebase silently, cross-checks understanding with user (3 interactions), generates docs. See [Existing Project Onboarding](/docs/guides/existing-project-onboarding).

**Path C (template/foundation):** Reads existing CLAUDE.md, docs/, .claude/agents/ — delegates implementation to template agents, tags example code. 1 interaction only. See [Existing Project Onboarding](/docs/guides/existing-project-onboarding).
...
Runs the full [Feature Lifecycle](/docs/architecture/feature-lifecycle) (phases 0-8) for the next feature on the roadmap.
...
See [Configuration](/docs/guides/configuration) for all available settings.
...
See [Rollback](/docs/guides/rollback) for details.

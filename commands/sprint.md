...
#### If argument is "show":
1. Read `.solo-dev/yaml/sprints.yaml`
2. Find sprint with status "active" (or most recent "planned")
3. Read `.solo-dev/yaml/features.yaml` for feature statuses
4. Display:
```
...
1. Read `.solo-dev/yaml/features.yaml` — filter features where `status == "QUEUED"`
2. Check dependencies: only show features where all `depends_on` feature IDs have `status == "COMPLETE"`
3. Read `.solo-dev/yaml/backlog.yaml` — show items that could be promoted to features
4. Read `.solo-dev/yaml/sprints.yaml` — get next sprint ID (S1, S2, etc.)
...
5. Create sprint entry in `.solo-dev/yaml/sprints.yaml`:
...
6. Update each feature in `.solo-dev/yaml/features.yaml`: set `effort_estimate`

7. Regenerate markdown: `bash ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/yaml-to-markdown.sh .solo-dev/yaml/sprints.yaml` and `bash ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/yaml-to-markdown.sh .solo-dev/yaml/features.yaml`
